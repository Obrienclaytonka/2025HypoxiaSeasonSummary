...
```{r, fig_align = 'center', fig.cap = '<i> Interactive map of LIS survey stations.  Click on each station to find information on sampling index period for that station. UCONN LISICOS Buoy Stations in Norwalk Harbor, Thames River, and the Eastern Sound have been discontinued. </i>'}
##install.packages("sf")
...
mRefPts <-leaflet() %>% 
  ...
  setView(lng = -72.799, lat = 41.126, zoom = 8) %>%   
...
<div class = 'itable'>
```{r, fig.align='center', tab.cap = '<i> Summary of sampling events during the 2025 Hypoxia Season</i>'}
...
```{r, fig.align = 'center', fig.cap = '<i> 2025 Hartford, CT Air Temperature Departures from Normal. Source: NowData- NOAA Online Weather Data, Daily data for a month https://www.weather.gov/wrh/Climate?wfo=box  </i>'}
library(ggplot2)
library(readxl)
...
<br>
Precipitation across the Long Island Sound region varied over the summer season. In Hartford, May and July was wetter than normal while June and August were drier than normal. (Normal values are calculated by NOAA's National Center for Environmental Information for the 30 year period from 1991-2020. Data can be obtained by visiting the [NOAA Online Weather Data portal](https://www.weather.gov/wrh/Climate?wfo=okx).) This lead to the summer of 2025 being close to average for Hartford with 12.39 inches of precipitation falling compared to the normal 12.66 inches. Islip also received close to average precipitation (8.05 inches in 2025 compared to the average 11.50 inches). Bridgeport, however, only received 21% of the normal amount of precipitation (2.34 inches versus 11.07 normally) making 2025 one of the driest on record.  

```{r, fig.align='center', fig.cap = '<i> 2025 Bridgeport, CT precipitation departures from normal. Source: NowData- NOAA Online Weather Data, Daily data for a month https://www.weather.gov/wrh/Climate?wfo=okx </i>'}
library(ggplot2)
library(readxl)
...
```
<br>
Wilson et. al., (2008) documented that wind direction can impact hypoxia in LIS, with winds approaching 203ºT producing maximum stratification and hypoxia, while winds out of the northeast contribute to decreased hypoxic area. Duvall et.al., (2023) suggest, after examining continuous buoy data, that "horizontal advection of DO by tides from the Upper East River into Western LIS" is responsible for modulating hypoxia.      
...
```{r, echo=FALSE, out.width="150%", fig.align = 'center', fig.cap = '<i> Windrose graphs of wind speed and direction from four regional airports around Long Island Sound during the summer of 2025. </i>'}
...
```{r, fig.align='center', fig.cap = '<i> Windrose graph of wind speed and direction from Western Long Island Sound LISICOS Buoy during the summer of 2025. </i>' }
...
data <- read.csv("data/WLIS_MET_2025.csv")


WRose <-windRose(data, ws = "ws", wd = "wd", 
                 breaks = c(0, 5, 10, 15, 20, 25, 30), 
                 cols = "increment", 
                 paddle=FALSE, 
                 offset = 0, 
                 key.position = "right", 
                 legend.title ="WESTERN LONG ISLAND SOUND BUOY n/counts of winds blowing from the indicated direction n/ Date Range: 2025-06-01 through 2025-09-01")


png("WLIS_Windrose.png", width = 800, height = 600)
##print(WRose)
##dev.off()
...
```{r,fig.align='center', fig.cap = '<i> Interactive map of average annual surface and bottom water temperatures measured in Long Island Sound. Scroll over the data points to see values. Use the slider to zoom to a particular time period.  </i>'}
...
widget3 <- dygraph(df, main = "Yearly Average Temperature of Long Island Sound") %>%
  ...
  dyAxis ("x", drawGrid = FALSE, axisLabelFormatter = JS("function(d) { return d.getFullYear() }"))%>%
...
```{r, fig.align='center', fig.width=7, fig.height=3.0, fig.cap = '<i> Interactive map of surface and bottom water temperatures measured during CT DEEP water quality cruises at Station A4 in Western Long Island Sound. Scroll over the data points to see values. Use the slider to zoom to a particular time period.  </i>'}
library (dygraphs)
library (readxl)
...
```{r,fig.align='center', fig.width=7, fig.height=3.0, fig.cap = '<i> Interactive map of surface and bottom water temperatures measured during CT DEEP water quality cruises at Station E1 in Central Long Island Sound. Scroll over the data points to see values. Use the slider to zoom to a particular time period.  </i>'}
library (dygraphs)
library (readxl)
...
```{r,fig.align='center', fig.width=7, fig.height=3.0, fig.cap = '<i> Interactive map of surface and bottom water temperatures measured during CT DEEP water quality cruises at Station M3 in Eastern Long Island Sound. Scroll over the data points to see values. Use the slider to zoom to a particular time period.  </i>'}
...
The Long Island Sound Partnership is utilizing a Generalized Additive Model (GAM) fitted to water quality data collected by CTDEEP to predict or forecast the extent of hypoxia. The forecast is part of a broader [Hypoxia Communications Toolkit](https://lispartnership.org/our-vision-and-plan/clean-waters-and-healthy-watersheds/reduce-nutrients/hypoxia-toolkit/). The 2025 forecast predicted that the maximum hypoxic area would be 79 +/- 40 square kilometers (31 square miles) and would occur near August 15. This [Story Map](https://storymaps.arcgis.com/stories/5a83ce1974bd472f8df111a3e2ad3fc8) describes the forecast tool and gives a history of the Partnership's efforts to reduce hypoxia and improve water quality.  

```{r, echo=FALSE, out.width="80%", fig.align = 'center', fig.cap = '<i> Forecasted maximum area of LIS Hypoxia in 2025 based on EPA LISP Generalized Additivge Model</i>'}

knitr::include_graphics("images/forecast_tool_hypoxia_depiction_2025.png")
...
Explore LIS OA data with this 
<a href="https://1oyc7t-obrienclaytonka.shinyapps.io/CTDEEP_LIS_Ocean_Acidification/" class="btn btn-primary" role="button"target="_blank" rel="noopener noreferrer">Interactive Data Dashboard</a>


```{r, echo=FALSE, fig.align = 'left', fig.cap = '<i> Interactive Coastal acidification Data Dashboard. Dashboard created by Kaleb Boudreaux after concept created by Goes Lab at Columbia University </i>'}

##[Interactive Coastal Acidification Data ##Dashboard](https://1oyc7t-obrienclaytonka.shinyapps.io/CTDEEP_LIS_Ocean_Acidification/){:target="_blank"}


##knitr::include_app("https://1oyc7t-obrienclaytonka.shinyapps.io/CTDEEP_LIS_Ocean_Acidification/", height = "1300px")
...
## **References**
<hr>
*Barrett LJ, Vlahos P, McGuinness MA, Whitney MM and Vaudrey JMP (2024) Droughts and deluges: changes in river discharge and the carbonate chemistry of an urbanized temperate estuary. Front. Mar. Sci. 11:1398087. doi:10.3389/fmars.2024.1398087.*
